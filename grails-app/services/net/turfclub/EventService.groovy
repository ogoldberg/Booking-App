...
class EventService {
    ...
    def featuredEventsAndBookings() {
        ...
        confEvents.each { confEvent ->
            ...
            featuredEventsAndBookings.add(individualEventMap)
        }

        return featuredEventsAndBookings
    ...
    def featuredEvents() {
        ...
        // todays events, we're not limited to events that are after today.hour
        def todayAt12am = Date.parse('yyyy-MM-dd', today.format('yyyy-MM-dd'))
        def todaysEvents = Event.findAllByEventDateGreaterThan(todayAt12am + 1).sort {
            it.eventDate
        }

        def confEvents = []
        featuredEvents.each { 
            if(it.bookings.find { booking -> booking.confirmed == true && booking.featured == true }) {
                confEvents.add(it)
            }
        }
